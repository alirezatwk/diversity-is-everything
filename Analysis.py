...
class Simulate():
    ...
    def simulate(self):
        ...
        if self.SAVE_FE:
            for REW_NAME in self.REWARDS_NAME:
                for i in range(len(self.REWARD_PROB_ARMS)):
                    TS_Policy = TS_Policies[REW_NAME][i, :, :]
                    main_TS_Policy =  ActionsResult(repetition = self.REPETITION, trials=self.EPISODE_MAX_LENGTH)
                    main_TS_Policy.set_actions(TS_Policy)

                    os.makedirs(self.SAVE_PATH + f"/TS_Policy/{self.SEED}_{REW_NAME}/", exist_ok = True)
                    with open(self.SAVE_PATH + f'/TS_Policy/{self.SEED}_{REW_NAME}/{self.AGENTS_ID[0]}_TS_Action_{i}.pkl', 'wb') as f:
                        pickle.dump(main_TS_Policy, f)

            for REW_NAME in self.REWARDS_NAME: 
                for i in range(self.AGENTS_COUNT):
                    for j in range(len(self.REWARD_PROB_ARMS)):
                        pi_star = Pi_Stars[REW_NAME][i,j,:,:]
                        main_pi_star = ActionsResult(repetition = self.REPETITION, trials=self.EPISODE_MAX_LENGTH)
                        main_pi_star.set_actions(pi_star)
                        os.makedirs(self.SAVE_PATH + f"/Pi_Star/{self.SEED}_{REW_NAME}/", exist_ok = True)
                        with open(self.SAVE_PATH + f'/Pi_Star/{self.SEED}_{REW_NAME}/{self.AGENTS_ID[0]}_Pi_Star_{self.AGENTS_ID[i]}_Action_{j}.pkl', 'wb') as f:
                            pickle.dump(main_pi_star, f)
    ...
    def visualize(self):
        ...
        for REW_NAME in self.REWARDS_NAME: 
            ...
            visualizer = ProbabilityOfChoosingActionsVisualizer(
                n_actions= len(self.REWARD_PROB_ARMS),
                max_trial=self.PLOT_MAX_LENGTH,
                experiment_name = f"Selection Actions[{self.EXPERIMENT_NAME}_{self.SEED}_{REW_NAME}]",
        ...
        if self.SAVE_FE:
            ...
            for REW_NAME in self.REWARDS_NAME:
                folder_path = self.SAVE_PATH + f"/TS_Policy/{self.SEED}_{REW_NAME}/" 
                visualizer5 = ActionPolicyVisualizer(
                                                    max_trial = self.PLOT_MAX_LENGTH,
                                                    experiment_name =  f"TS_Policy[{self.EXPERIMENT_NAME}_{self.SEED}_{REW_NAME}]",
                                                    data_path = folder_path,
            ...
            for REW_NAME in self.REWARDS_NAME:
                folder_path = self.SAVE_PATH + f"/Pi_Star/{self.SEED}_{REW_NAME}/" 
                visualizer6 = ActionPolicyVisualizer(
                                                    max_trial = self.PLOT_MAX_LENGTH,
                                                    experiment_name =  f"Pi_Star[{self.EXPERIMENT_NAME}_{self.SEED}_{REW_NAME}]",
                                                    data_path = folder_path,
