...
class ActionsResult:
    ...
    def probability_of_choosing_action(self, desired_action: int) -> np.array:
        return np.mean(self.actions == desired_action, axis=0), 2 * np.std(self.actions == desired_action, axis=0)/ np.sqrt(self.actions.shape[0])

    def get_actions(self):
        return np.mean(self.actions, axis=0), 2 * np.std(self.actions, axis=0)/ np.sqrt(self.actions.shape[0])