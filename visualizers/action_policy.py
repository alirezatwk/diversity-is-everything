...
class ActionPolicyVisualizer:
    ...
    def _read_results(self) -> dict:  # TODO: Use the correct type hint.
        ...
        for file_name in sorted(os.listdir(self.data_path)):
            ...
            with open(os.path.join(self.data_path, file_name), 'rb') as pickle_file:
                ...
                agents_probabilities[name], agents_errors[name] = agent_actions.get_actions()
    ...
    def _get_figure(self, agents_pdf: pd.DataFrame): # TODO: use type hint
        figure_with_error_bars = px.line(agents_pdf, x='trial', y='action policy', error_y='error',
                                         color='agent', title= self.experiment_name)
        fig = px.line(agents_pdf, x='trial', y='action policy', color='agent', title= self.experiment_name)
...
if __name__ == '__main__':
    visualizer = ActionPolicyVisualizer(
        best_action=0,
        max_trial=100,
