...
class ProbabilityOfChoosingActionsVisualizer:
    def __init__(self, n_actions: int, max_trial: int, experiment_name:str, data_path: str, write_path: str):
        ...
        for file_name in sorted(os.listdir(self.data_path)):
            ...
            with open(os.path.join(self.data_path, file_name), 'rb') as pickle_file:
                ...
                for i in range(self.n_actions):
                    agents_probabilities[name][i], agents_errors[name][i] = agent_actions.probability_of_choosing_action(i)
        ...
        for agent in agents_probabilities:
            for i in range(self.n_actions):
                ...
                probabilities_pdf = pd.DataFrame(
                    ...
                    columns=['probability of choosing action'],
                )
                probabilities_pdf['trial'] = list(range(agent_max_trial))
                probabilities_pdf['agent'] = agent + f'_action {i}'
    ...
    def _get_figure(self, agents_pdf: pd.DataFrame): # TODO: use type hint
        figure_with_error_bars = px.line(agents_pdf, x='trial', y='probability of choosing action', error_y='error',
                                         color='agent', title= self.experiment_name)
        fig = px.line(agents_pdf, x='trial', y='probability of choosing action', color='agent', title= self.experiment_name)
