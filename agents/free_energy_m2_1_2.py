...
class FreeEnergySocialAgent_M2_1_2(AgentBase):
    def __init__(
            self,
            id: str,
        ...
        super(FreeEnergySocialAgent_M2_1_2, self).__init__(
            id=id,
    ...
    def _f(self, x: float, action_ind: int) -> float:
        ...
        #         f = f * beta.cdf(x, a = self.hp1[a_ind], b = self.hp2[a_ind])      
            f = np.prod(beta.cdf(x, loc = self.hp1, scale= self.hp2)) 
            f = f * beta.pdf(x, loc = self.hp1[action_ind], scale = self.hp2[action_ind])
            f = f / beta.cdf(x, loc = self.hp1[action_ind], scale = self.hp2[action_ind] + self.epsilon)
    ...
    def _calculate_TS_policy(self, n_samples = 1000) -> np.array:
        ...
        #     TS_policy[act] += 1     
        TS_policy = [integrate.quad(lambda x: self._f(x,a), self.lower_bound[a], self.upper_bound[a])[0] for a in range(self.n_actions)]
